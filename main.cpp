...
class vanishingPt
{
    ...
    vanishingPt()
    {
        ...
        minlength /= 10. ; //added for debugging ...must be removed later
        int flag=0;
        while( cap.isOpened() )//check if camera/ video stream is available
        {
            ...
            int k = cv::waitKey(1);
            cout<<"fps: "<<1/(double(end-begin)/CLOCKS_PER_SEC)<<";k="<<k<<endl;
    ...
    void makeLines(int flag)
    {
        ...
        A = zeros<mat>(points_size, 2);
        b = zeros<mat>(points_size, 1);
    ...
    void eval()
    {
        ...
        for(int i=0; i<points_size; i++)
        {
            for(int j=0; j<points_size; j++)
            {
                ...
                temperr = 0;
                //summation of errors
                for(int i=0; i<(int)error.n_rows ; i++)
